...
const objLat = (obj) => {
    ...
    const templateText= `my name is ${firstName} ${lastName} I am ${obj.age} YO, and I love ${obj.hobby}.`;
    return templateText;
};
...
const cvFormatter = (arr) => {
    ...
    for(let i=0;i<arr.length;i++){
        ...
        if(arr[i].yearsOfExperience > 1){
            formattedCVs.push({fullName,tech: arr[i].tech});
        }
...
const applicationsStatics = (arr) => {
    ...
    let result = {
        ...
        dotNet_devs: 0,
        java_devs: 0,
         totalApplicants: 0,//=====>arr.length
        rejectedApplicants: 0,
    }
    for(let i=0;i<arr.length;i++)
    {
        const{firstName,lastName,yearsOfExperience}=arr[i];

    
    if(!firstName||!lastName||yearsOfExperience<=1){//======>add else if or switch to add numOf courece
...
const classesAvg = (data) => {
    for(const itemOut of data.grades){
        for(const itemIn of itemOut.classes){
            let temp=0;
            for(let i=0;i<=itemIn.classScores.length - 1;i++){
                temp+=itemIn.classScores[i];
            }
            itemIn.avg=Math.floor(temp/itemIn.classScores.length);
        }
    }
    return data;
};
